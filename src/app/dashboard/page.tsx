...
export default async function DashboardPage() {
  ...
  const isExecutive = role === "CEO" || role === "PM";
  ...
  try {
    ...
    const [cRows]: any = await pool.query("SELECT COUNT(*) as cnt FROM tasks WHERE status = 'Completed'");
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="rounded-2xl bg-gradient-to-r from-slate-900 via-slate-800 to-sky-950 p-8 text-white shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-2">
          <span className="inline-block px-3 py-1 rounded-full bg-sky-500/20 text-sky-300 text-xs font-bold uppercase tracking-wider border border-sky-500/30">
            {role} Executive Portal
          </span>
          <h1 className="text-3xl font-extrabold tracking-tight">
            Welcome back, {session?.user?.name}!
          </h1>
          <p className="text-slate-300 text-sm max-w-2xl">
            Unitglo Solutions Employee Progress & Task Tracking System is actively monitoring project delivery timelines, test verification queues, and delay metrics.
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm hover:shadow-md transition-all group">
          ...
          <p className="text-xs text-slate-500 mt-1">Active company initiatives</p>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm hover:shadow-md transition-all group">
          ...
          <p className="text-xs text-slate-500 mt-1">Managed tasks across team</p>
        </div>
      ...
      {/* CEO & PM Executive Filter Dashboard */}
      {isExecutive && <CEOFilterDashboard />}
    </div>
  );
}
